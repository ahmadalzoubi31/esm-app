...
export function UserPermissions({ form }: UserPermissionsProps) {
  return (
    <form.Subscribe
      ...
      children={([roles]: any) => (
        <UserPermissionsContent form={form} selectedRoleIds={roles || []} />
      )}
    />
  )
}
...
function UserPermissionsContent({
  form,
  selectedRoleIds,
}: UserPermissionsProps & { selectedRoleIds: string[] }) {
  const { data: allPermissions, isLoading: isLoadingAll } =
    usePermissionsQuery()
  const { data: rolesList } = useRolesQuery()

  // Fetch permissions for each selected role
  ...
  // Aggregate inherited permissions: PermissionID -> Array of Role Names
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Manage additional direct permissions. Select permissions from the
...
function PermissionSelector({
  ...
  const { inherited, groupedAvailable } = useMemo(() => {
    ...
    allPermissions.data.forEach((permission) => {
      ...
      // If already selected directly, exclude from "available" list in menu
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <Popover
          ...
          onOpenChange={(isOpen) => {
            setOpen(isOpen)
            if (!isOpen) setActiveCategory(null) // Reset category on close
          }}
        >
                    ...
                    <CommandGroup heading={activeCategory}>
                      {groupedAvailable[activeCategory] ? (
                        groupedAvailable[activeCategory].map((permission) => (
                          <CommandItem
                            key={permission.id}
                            value={`${permission.key} ${permission.description}`} // Search both key and description
